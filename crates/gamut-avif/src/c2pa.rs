...
///   [`gamut_isobmff::write`] rejects one at or beyond 4 GiB with [`Error::Unsupported`].
///   Measured on a 64-bit host with this framing, the largest `len` that clears that check is
///   `4_294_967_250` and `4_294_967_251` is refused, and a `len` just under it aborts in the
///   allocator while the writer copies the payload into the output buffer. So above the writer's
///   bound lies the writer's error, and below it — for a length the machine has no memory for —
///   lies the allocator's own limit, which no fallible API here can intercept.
...
///   [`Error::InvalidInput`] and the writer's `Unsupported` never fires. Not measured on such a
///   target; it follows from the two constants.
